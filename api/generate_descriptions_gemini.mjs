/**
 * Generate AI Photo Descriptions using Google Vision (Gemini 1.5 Flash Vision API).
 *
 ...
 *
 * Usage:
 *   node --env-file=.env api/generate_descriptions_gemini.mjs
 */
...
async function getWorkingModelName() {
    try {
        ...
        if (listRes.ok && Array.isArray(listData.models)) {
            ...
            if (visionModel) {
                // name comes as "models/gemini-..." -> strip prefix if present
...
async function describeImageWithGoogleVision(imagePath, altText, modelName = ACTIVE_MODEL) {
    ...
    // Try current model, with fallback models if 404
    const modelsToTry = [
        modelName,
        'gemini-3.6-flash',
        'gemini-3.5-flash',
...
const DELAY_MS = 12500; // ~5 requests per minute for Free Tier
    ...
    while (!success && attempts < 5) {
        ...
        try {
            ...
            if (desc) {
                ...
                // Save incrementally after each photo so progress is preserved!
