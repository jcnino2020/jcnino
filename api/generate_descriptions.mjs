 ...
 *
 * Usage:
 *   node --env-file=.env api/generate_descriptions.mjs
 *
...
if (!API_KEY) {
    console.error('❌ DEEPSEEK_API_KEY is not set. Run with: node --env-file=.env api/generate_descriptions.mjs');
    process.exit(1);
}
...
for (let i = 0; i < missing.length; i += BATCH_SIZE) {
    ...
    while (!ok && attempts < 3) {
        ...
        try {
            ...
            // Parse lines like: `GH-01.JPG: description`
...
const newlyAdded = withDesc.filter(p => p.desc && !allPhotos.find(orig => orig.file === p.file && orig.desc));
...
for (const p of withDesc) {
    ...
    // Match the exact original object line in the file.
...
const stillMissing = withDesc.filter(p => !p.desc).map(p => p.file);
if (stillMissing.length) {
    console.log(`⚠️  Still missing descriptions for: ${stillMissing.length} photos`);
}